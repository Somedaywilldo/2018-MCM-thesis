...
cross_file = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/cross_Sin.shp")
cross = cross_file.shapes()
road_file = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/Sin_84.shp")
...
for tt in Type: 
	city_file = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/Ireland_%s.shp"%tt)
	city = city_file.shapes()
	less_file = []
	less_file = shapefile.Writer(shapefile.POINTM) #3是直线，1是点
	less_file.autoBalance = 1
	less_file.field('Type')
	for i in range(len(city)):
		if(i%Kless==0):
			less_file.record(tt)
			less_file.point(city[i].points[0][0],city[i].points[0][1])
	print(tt,len(city)/Kless)
	less_file.save("/Users/mac/Desktop/Ireland_Road/%s_station_point"%tt)
...
for i in range(len(cross)):
	flag=0
	for j in range(i):
		now_x = cross[i].points[0]
		now_y = cross[j].points[0]
		if(geo_dist(now_x,now_y)<=0.1):
			flag=1
			break
	if(flag):
		continue
...
cross_unique_file.save("/Users/mac/Desktop/Ireland_Road/cross_unique_road")


######################################
road_unique_file = []
road_unique_file = shapefile.Writer(shapeType=3) #3是直线，1是点
road_unique_file.autoBalance = 1
road_unique_file.field('Unique')
...
for i in range(len(road)):
	flag=0
	now_st = road[i].points[0]
	now_en = road[i].points[-1]
	for j in range(i):	
		#
		old_st = road[j].points[0]
		old_en = road[j].points[-1]
		#
		if(geo_dist(now_st,old_st)<=0.1 and geo_dist(now_en,old_en)<=0.1):
			flag=1
			break
	if(flag):
		continue
	else:
		road_unique.append([now_st,now_en])

for i in road_unique:
	old_st = i[0]
	old_en = i[1]
	now_st = []
	now_en = []
	for j in cross_unique:
		now_point = list(j[0])
		if(geo_dist(old_st,now_point)<=0.2):
			now_st = old_st
	for j in cross_unique:
		now_point = list(j[0])
		if(geo_dist(old_en,now_point)<=0.2):
			now_en = old_en
	if(now_st != [] and now_en != []):
		print(now_st,now_en)
		road_unique_file.record("Unique")
		road_unique_file.line(parts=[[ now_st,now_en ]])

road_unique_file.save("/Users/mac/Desktop/Ireland_Road/road_unique")
...
line_road_file.save("/Users/mac/Desktop/Ireland_Road/line_road")

line_road = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/line_road.shp")
...
line_road_dist_file.save("/Users/mac/Desktop/Ireland_Road/line_dist_road")
...
for i in state_code:
	...
	state_point_file.save("/Users/mac/Desktop/Ireland_Road/%s_point"%i)
	#state_road_shp:


line_dist_road_file = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/line_dist_road.shp")
...
for i in state_code:
	...
	state_road_file.save("/Users/mac/Desktop/Ireland_Road/%s_road"%i)
...
for state_code_i in range(int(len(state_code)) ):
	...
	state_point_file = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/%s_point.shp"%state_name_i)
	state_point = state_point_file.shapes()
	state_road_file = shapefile.Reader("/Users/mac/Desktop/Ireland_Road/%s_road.shp"%state_name_i)
	...
	state_dense_point_file.save("/Users/mac/Desktop/Ireland_Road/%s_dense_point"%state_name_i)
	#
	#画在shapefile上面：
	...
	#
	state_station_point_file.save("/Users/mac/Desktop/Ireland_Road/%s_station_point"%state_name_i)
	#统计城乡分布
...
urban_file.save("/Users/mac/Desktop/Ireland_Road/Urban_station_point")
...
suburban_file.save("/Users/mac/Desktop/Ireland_Road/Suburban_station_point")
...
rural_file.save("/Users/mac/Desktop/Ireland_Road/Rural_station_point")
